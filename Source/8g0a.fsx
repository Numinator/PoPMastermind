...
let realCode = makeCode (askPlayerType ())
(* validate: Part of the guess loop  *)
let validate (rc: code) (c: code) :answer =
  let mutable i   = 0
  let mutable b   = 0
  let mutable w   = 0
  let listCode    = [Red;Green;Yellow;Purple;White;Black] //en smule unødvendig
  ...
  for i=0 to 3 do 
    if rc.[i] = c.[i] then
      b <- b + 1
  match rc.[i] with
  | col when col = listCode.[0] -> histRc.[0]+1
  | col when col = listCode.[1] -> histRc.[1]+1
  | col when col = listCode.[2] -> histRc.[2]+1
  | col when col = listCode.[3] -> histRc.[3]+1
  | col when col = listCode.[4] -> histRc.[4]+1
  | col when col = listCode.[5] -> histRc.[5]+1
    
  match c.[i] with
  | col when col = listCode.[0] -> histC.[0]+1
  | col when col = listCode.[1] -> histC.[1]+1
  | col when col = listCode.[2] -> histC.[2]+1
  | col when col = listCode.[3] -> histC.[3]+1
  | col when col = listCode.[4] -> histC.[4]+1
  | col when col = listCode.[5] -> histC.[5]+1 
